...
from Utils.Dataloader_train import ZSData
...
def get_val_csv(modellist, dataloaders, phase, criterion, optimizer):
    ...
    csv_dict = {"labels": all_labels.astype(int), "predictions": all_predicts.astype(int), 'value0': all_vl_1, 'value1': all_vl_2, 'value2': all_vl_3}
...
def prepare_data(dataset_path, task, batch_size, stat):
    ...
    if stat == 'train':
        train_dataset = ZSData(os.path.join(dataset_path, "train"), task, transforms=utils.transform_train, transforms_512 = utils.transform_train_512, bi=True, padding=0)
        valid_dataset = ZSData(os.path.join(dataset_path, "test"), task, transforms=utils.transform_valid, transforms_512 = utils.transform_valid_512, bi=True, padding=0)
        ...
        test_dataset = ZSData(os.path.join(dataset_path, str(args.foldn), 'test'), task, transforms=utils.transform_valid, transforms_512 = utils.transform_valid_512, bi=True, padding=0)
...
if __name__ == '__main__':
    ...
    # 15  CUDA_VISIBLE_DEVICES=1,2  python /home/21/zihan/Storage/lympho/code/train_ntl.py --TrainFolder "/home/21/zihan/Storage/lympho/splited/splited_712_noverlap_20x"  --Comment 'lr0.001,Decay0.0005,SGD,cross,Pre,256,rs224,20x,alpha0.5,ntl' --Pretrain --FoldN 1 --BatchSize 8 --Epoch 100 --Resume 0 --Stat test
    # 15  CUDA_VISIBLE_DEVICES=0,1  python /home/21/zihan/Storage/lympho/code/train_ntl.py --TrainFolder "/home/21/zihan/Storage/lympho/patch_liuyuan_nt_test/"  --Comment 'lr0.001,Decay0.0005,SGD,cross,Pre,256,rs224,20x,alpha0.5,ntl' --Pretrain --FoldN 3 --BatchSize 32 --Epoch 100 --Resume 0 --Stat test
    # 15  CUDA_VISIBLE_DEVICES=5,7  python /home/21/zihan/Storage/lympho/code/train.py --TrainFolder "/home/21/zihan/Storage/lympho/splited/splited_712_noverlap_20x"  --Comment 'test_problem' --Pretrain --FoldN 1 --BatchSize 256 --Epoch 200 --Resume 0
    ...
    parser.add_argument("--Task", dest='task', type=str, default = 'ntl', help='the task of training')
